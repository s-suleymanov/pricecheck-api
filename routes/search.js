...
const BRAND_MODEL_NAME_SQL =
  "lower(trim(concat_ws(' ', coalesce(c.brand,''), coalesce(c.model_name,''))))";
...
router.get("/api/suggest", async (req, res) => {
  ...
  WITH picked AS (
    ...
    WHERE c.model_number IS NOT NULL
      ...
      AND (
        ...
        OR ${BRAND_MODEL_NAME_SQL} LIKE $1
      )
    ORDER BY
      upper(btrim(c.model_number)),
      COALESCE(NULLIF(lower(btrim(c.version)), ''), ''),
  ...
  SELECT
    btrim(model_name) AS model_name,
...
`;
  ...
  if (qLower.length >= 2) {
    ...
    modelResults = rows
  .filter((r) => r && (r.model_name || r.value))
  .map((r) => {
    ...
    const modelNumber = String(r.value || "").trim();
