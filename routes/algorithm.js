...
const HOME_FEED_LOCK_KEY = 8801001;
const HOME_FEED_TARGET_ROWS = 1000;

async function hasTodayHomeFeed(client) {
  const q = await client.query(`
    SELECT 1
    FROM public.home_feed_daily
    WHERE feed_date = (now() AT TIME ZONE 'America/Los_Angeles')::date
    LIMIT 1
  `);
...
async function rebuildTodayHomeFeed(client) {
  await client.query(`
    DELETE FROM public.home_feed_daily
    WHERE feed_date = (now() AT TIME ZONE 'America/Los_Angeles')::date
  `);

  await client.query(`
    INSERT INTO public.home_feed_daily (
      feed_date,
      position,
      key,
      title,
    ...
    sa AS (
      SELECT
        key,
        ARRAY_AGG(store_key ORDER BY bp ASC, store_key ASC) AS stores
      FROM sb
      GROUP BY key
    ),
    ...
    grouped AS (
        ...
        (
          CASE
            WHEN lower(COALESCE(g.category, '')) LIKE '%earbud%'
              OR lower(COALESCE(g.category, '')) LIKE '%headphone%'
              OR lower(COALESCE(g.title, '')) LIKE '%earbud%'
              OR lower(COALESCE(g.title, '')) LIKE '%headphone%'
              OR lower(COALESCE(g.title, '')) LIKE '%airpods%'
              OR lower(COALESCE(g.title, '')) LIKE '%buds%'
            THEN 25000
            ELSE 0
          END

          + CASE
              WHEN g.last_seen > now() - interval '3 days' THEN 12000
              WHEN g.last_seen > now() - interval '7 days' THEN 9000
              WHEN g.last_seen > now() - interval '14 days' THEN 6000
              WHEN g.last_seen > now() - interval '30 days' THEN 3000
              ELSE 0
            END
      ...
      WHERE g.store_count >= 2
        ...
        AND lower(btrim(g.title)) <> 'product'
        AND NULLIF(btrim(g.brand), '') IS NOT NULL
        AND NULLIF(btrim(g.category), '') IS NOT NULL
        AND NULLIF(btrim(g.image_url), '') IS NOT NULL
    ),

    ranked AS (
      SELECT
        (now() AT TIME ZONE 'America/Los_Angeles')::date AS feed_date,
        (ROW_NUMBER() OVER (
          ORDER BY
            score DESC,
    ...
    WHERE position < $1
    ORDER BY position
  `, [HOME_FEED_TARGET_ROWS]);
}

async function ensureTodayHomeFeed(client) {
  if (await hasTodayHomeFeed(client)) return;
  ...
  try {
    if (!(await hasTodayHomeFeed(client))) {
      await rebuildTodayHomeFeed(client);
    }
  } finally {
    await client.query(
...
async function getLatestHomeFeedDate(client) {
  const q = await client.query(`
    SELECT feed_date
    FROM public.home_feed_daily
    WHERE feed_date <= (now() AT TIME ZONE 'America/Los_Angeles')::date
  ...
  return q.rows[0]?.feed_date || null;
}

async function sendHomeFeed(req, res) {
  const limit = clampInt(req.query.limit ?? req.body?.limit, 6, 100, 24);
  const offset = clampInt(req.query.offset ?? req.body?.offset, 0, 5e6, 0);

  const client = await pool.connect();

  try {
    await ensureTodayHomeFeed(client);

    const feedDate = await getLatestHomeFeedDate(client);

    if (!feedDate) {
      return res.json({
        ok: true,
        source: "daily_cache_empty",
        limit,
        offset,
        has_signals: false,
        results: []
    ...
    console.error("home_feed cache error:", err);
...
router.get("/api/home_feed", sendHomeFeed);
router.post("/api/home_feed", sendHomeFeed);
